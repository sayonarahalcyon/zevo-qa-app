...
6. **Existing database?** If `qa_entries` was created before the "Mark as test audit" feature, run `sql/migrations/2026_09_08_add_is_test_to_qa_entries.sql` once in the Supabase SQL editor before deploying this version of the code (new installs don't need it — `sql/schema.sql` already includes the column). Likewise, if `disputes` was created before it had an `is_test` column, run `sql/migrations/2026_09_18_add_is_test_to_disputes.sql`.
