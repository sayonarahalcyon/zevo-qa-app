...
- **Questions & disputes**: agents submit questions/disputes directly on the audit, from My Dashboard, instead of a separate Google Form — see the `disputes` table in `sql/schema.sql` and `lib/disputes.py`. Reviewers respond from QA Log's "Questions & disputes" panel; an actual score change still goes through the existing "Edit score" → reason "Dispute" flow (`qa_entries.edit_log`), which the panel links straight into.
